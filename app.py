...
def load_price(ticker, force_market=None):
    ...
    모든 소스 결과를 clean_ohlcv로 통과시키고, 유효 종가가 확보될 때까지 다음 소스로 넘어간다.
    """
    t = str(ticker).strip().upper()
    log = []

    def try_source(label, fn):
        """(정리된 df, 라벨) 또는 (None, None)"""
        try:
            raw = fn()
        except Exception as ex:
            log.append(("일봉", "재시도", f"{label}: {type(ex).__name__}"))
            return None, None
        if raw is None or len(raw) == 0:
            log.append(("일봉", "재시도", f"{label}: 빈 응답"))
            return None, None
        cleaned, notes = clean_ohlcv(raw)
        if cleaned is None or len(cleaned) < 60:
            log.append(("일봉", "재시도",
                        f"{label}: 정리 후 {0 if cleaned is None else len(cleaned)}행 "
                        f"({'; '.join(notes) if notes else '사유 미상'})"))
            return None, None
        last = float(cleaned["Close"].iloc[-1])
        if not np.isfinite(last) or last <= 0:
            log.append(("일봉", "재시도", f"{label}: 마지막 종가 이상({last})"))
            return None, None
        if notes:
            log.append(("일봉 정리", "부분", f"{label} · " + "; ".join(notes)))
        return cleaned, label

    is_kr = (force_market == "KR") or (force_market is None and is_kr_code(t))
    df, used = None, None

    if is_kr:
        cands = []
        if HAS_FDR:
            cands.append(("FinanceDataReader", lambda: fdr.DataReader(t)))
        if HAS_KRX:
            cands.append(("pykrx", lambda: krx.get_market_ohlcv_by_date(
                "19950101", ymd(bday()), t)))
        if HAS_YF:
            cands.append((f"yfinance {t}.KS",
                          lambda: yf.Ticker(t + ".KS").history(period="max", auto_adjust=True)))
            cands.append((f"yfinance {t}.KQ",
                          lambda: yf.Ticker(t + ".KQ").history(period="max", auto_adjust=True)))
        for lab, fn in cands:
            df, used = try_source(lab, fn)
            if df is not None:
                break
        if df is None:
            log.append(("일봉", "실패", "모든 소스에서 유효한 종가를 얻지 못했습니다"))
            return None, "KR", t, log
        nm, mismatch, nsrc = name_consensus(t)
        if mismatch:
            log.append(("종목명 교차검증", "부분", "소스별 이름 불일치 — " +
        ...
        log.append(("일봉", "성공",
                    f"{used} · {len(df):,}일 ({df.index[0]:%Y-%m-%d} ~ "
                    f"{df.index[-1]:%Y-%m-%d}) · 최종 종가 {df['Close'].iloc[-1]:,.2f}"))
        return df, "KR", name, log

    # ── 해외 ──────────────────────────────────────────────────────────────
    cands = []
    if HAS_YF:
        cands.append(("yfinance history",
                      lambda: yf.Ticker(t).history(period="max", auto_adjust=True)))
        cands.append(("yfinance download",
    ...
    for code, nm in (KR_INDICES if market == "KR" else US_INDICES):
        ...
        if got is None and HAS_FDR and market != "KR":
            try:
                ...
                pass
        cleaned, notes = clean_ohlcv(got, need_vol=False) if got is not None else (None, [])
        if cleaned is not None and len(cleaned) > 150:
            out[nm] = cleaned
            novol = bool(cleaned["Volume"].fillna(0).sum() == 0)
            log.append((f"지수 {nm}", "부분" if novol else "성공",
                        f'{len(cleaned):,}일 · 최종 {cleaned["Close"].iloc[-1]:,.1f}'
                        + (" · 거래량 없음(분산일 판정 제한)" if novol else "")
                        + (" · " + "; ".join(notes) if notes else "")))
        else:
            log.append((f"지수 {nm}", "실패",
                        "소스 응답 없음" if got is None else
                        f'정리 후 {0 if cleaned is None else len(cleaned)}행 '
                        + ("; ".join(notes) if notes else "")))
    return out, log
...
def build_context(tk, force_market=None):
    ...
    _last = float(dfd["Close"].iloc[-1]) if len(dfd) else float("nan")
    if not np.isfinite(_last) or _last <= 0:
        ctx["log"] = log + [("현재가", "실패", "마지막 종가가 유효하지 않습니다")]
        ctx["name"] = name
        return ctx
...
with TABS[3], guard("개별종목"):
    if not CTX.get("ok") or not D:
        ...
        with st.expander("종목 정보 검증 — 이 코드가 정말 이 종목인가",
            ...
            with st.expander("최근 시세 5행 (원본 확인)"):
                st.dataframe(df.tail(5).iloc[::-1], use_container_width=True)
                st.caption("종가가 비어 있거나 0이면 그 행은 자동으로 제외됩니다. "
                           "여기 값이 실제 시세와 다르면 데이터 소스 문제입니다.")
            if CTX.get("name_mismatch"):
    ...
    if st.button("스캔 실행", type="primary"):
        ...
        for i, t0 in enumerate(tks, 1):
            ...
            if not t:
                ...
                continue
            try:
                d, mk, nm, _lg = load_price(t, fm)
                if d is None or len(d) < 200 or not np.isfinite(float(d["Close"].iloc[-1])):
