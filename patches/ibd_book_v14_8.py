# v14.9 book: 2026-09-07/08 Checkup + list-change article + Pulse leaders
BOOK_REV_20260908 = 9
CHECKUP_BOOK_20260908 = True
CHECKUP_BOOK_V14_9 = True
...
def _C08(**kw):
    ...
    body = (f'Comp {kw.get("comp")} / EPS {kw.get("eps")} / RS {kw.get("rs")} / '
            ...
            f'fund {kw.get("funds_chg")}% / {kw.get("funds_up_q")}q. '
...
CHECKUP_BOOK_20260908_ROWS = [
    _C08(ticker="ANET", name="Arista Networks", group="COMPUTER-NETWORKING", group_rank=25, group_rs="A",
         ...
         inst_note="within-group 1 / group 25 A. RS 86 A/D D vs50 +5.9. not a new buy."),
    _C08(ticker="NOW", name="ServiceNow", group="COMPUTER SFTWR-ENTERPRISE", group_rank=8, group_rs="A+",
         ...
         funds_chg=-3, funds_up_q=0, ud=1.47, roe=14.2, debt=18, mcap="146.1 B", due="2026-10-28",
         inst_note="pivot 139.20. RS 60 funds -3 off52 -27 vs50 +20.7."),
    _C08(ticker="PBF", name="PBF Energy", group="OIL&GAS-REFINING/MKTG", group_rank=1, group_rs="A+",
         ...
         inst_note="SMR D EPS 72 vs50 +16.3. hold no add."),
    _C08(ticker="SHOP", name="Shopify", group="RETAIL-INTERNET", group_rank=8, group_rs="A+",
         ...
         inst_note="pivot 158.87 not reached. RS 70 funds -4."),
    _C08(ticker="SNOW", name="Snowflake", group="COMPUTER SFTWR-ENTERPRISE", group_rank=8, group_rs="A+",
         ...
         inst_note="Pulse volume leader. SMR C ROE neg debt 140 vs50 +13.4."),
    _C08(ticker="VEEV", name="Veeva Systems", group="MEDICAL-SYST/SOFTWR", group_rank=5, group_rs="A+",
         ...
         inst_note="Comp 99 A/D A+. RS 87 vs50 +25. no chase."),
    _C08(ticker="FIVE", name="Five Below", group="RETAIL-DISCOUNT", group_rank=136, group_rs="E",
         ...
         inst_note="IBD50 add. group 136 E. RS 84 vs50 +15.6. list != buy."),
    _C08(ticker="IOT", name="Samsara", group="COMPUTER SFTWR-ENTERPRISE", group_rank=8, group_rs="A+", exposure="60%-80%",
         ...
         inst_note="Comp 82 RS 70 EPS 64. below gate."),
]
...
LIST_CHANGES_20260904 = {
    ...
    "note": "list add != buy. WT cut from Notable. SNOW cut BigCap/Notable but Pulse volume leader.",
}

MARKET_PULSE_IMAGE_20260903 = {
    "date": "2026-09-03",
    "source": "MARKET PULSE.png",
    ...
    "ftd": "2026-06-02",
    "market_score": 65,
    "note": "Pulse is source of truth. Yahoo index volume is not IBD exchange volume.",
}
...
def ensure_book_seed(desk):
    ...
    if "market_pulse_20260903" not in lists:
        lists["market_pulse_20260903"] = MARKET_PULSE_IMAGE_20260903
        changed = True
    elif not (lists.get("market_pulse_20260903") or {}).get("leaders_up"):
        lists["market_pulse_20260903"].update({k: v for k, v in MARKET_PULSE_IMAGE_20260903.items() if not lists["market_pulse_20260903"].get(k)})
        changed = True
    if changed:
