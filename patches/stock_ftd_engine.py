...
def stock_distribution_days(df, lookback=25, min_drop=0.5, expire_up=0.05):
    """종목 분산일(매도일) 목록. 오닐 지수 규칙의 종목 이식.
    판정: 종가 하락폭 >= min_drop% AND 당일 거래량 > 전일 거래량.
    ...
    for i in range(1, len(d)):
        ...
        if chg <= -abs(min_drop) and vol > pvol:
            ...
            out.append({"date": d.index[i], "chg": float(chg), "close": close_i,
                        "vol": float(vol), "vol_vs_prev": float(vol / pvol) if pvol else np.nan,
